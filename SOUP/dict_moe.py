...
class ParetoWeightEnsemblingModule(nn.Module):
    def __init__(
        ...
        if router_hidden_layers == 1:
            ...
            if init_lambda is None:
                nn.init.eye_(self.gate[0].weight)
    ...
    def _set_preference_vector(self, perference_vector: Tensor):
        ...
        self.preference_vector = perference_vector.to(device)
    ...
    def _merge_state_dict(self):
        ...
        for param_idx, (name, params) in enumerate(
            ...
            routing_weights = routing_weights.to(expert_params.device)
        ...
        return merged_state_dict,routing_weights
    ...
    def forward(self, *args, **kwargs):
        ...
        if self._merged_state_dict is None:
            ...
            self._merged_state_dict, _ = self._merge_state_dict()
    ...
    def get_merged_model(self):
        ...
        """
        merged_state_dict,_ = self._merge_state_dict()
        model = deepcopy(self.base_model)
        model.load_state_dict(merged_state_dict)
        return model

    @staticmethod
    def set_preference_vector(model: nn.Module, preference_vector: Tensor):
        if isinstance(model, ParetoWeightEnsemblingModule):
            model._set_preference_vector(preference_vector)
        for name, module in model.named_children():
            if isinstance(module, nn.Module):
                ParetoWeightEnsemblingModule.set_preference_vector(
